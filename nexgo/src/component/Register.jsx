    ...
    <div className="min-h-screen w-full flex items-center justify-center bg-gradient-to-b from-background to-secondary/30 p-4 mt-20">
      <div className="w-full max-w-md flex flex-col overflow-hidden rounded-3xl bg-white/80 backdrop-blur-xl shadow-lg border border-border/40 transition-all duration-500">
        <div className="p-8 sm:p-10">
          ...
          <p className="text-center text-muted-foreground mb-8">Join our platform today</p>
          ...
          <form onSubmit={handleSubmit} className="space-y-5">
            ...
            <select name="role" value={formData.role} onChange={handleChange} className="w-full px-4 py-3 border rounded-xl" disabled={isLoading}>
              <option value="customer">Customer</option>
              <option value="restaurant">Restaurant</option>
              <option value="delivery">Delivery</option>
            </select>
            
            <button type="submit" disabled={isLoading} className="w-full bg-primary text-white py-3 px-4 rounded-xl font-medium flex items-center justify-center gap-2 transition-all duration-300 hover:bg-primary/90">
